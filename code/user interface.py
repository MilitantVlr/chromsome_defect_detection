...
class ObjectDetectionApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Chromosome Defect Detection using AI")
        self.root.geometry("800x800")  # Set window size

        style = ThemedStyle(root)
        style.set_theme("equilux")  # Set the theme for the whole app

        title_label = tk.Label(root, text="Chromosome Defect Detection using AI ", font=("Helvetica", 20))
        title_label.pack(pady=20)

        self.load_button = tk.Button(root, text="Load Image", command=self.load_image)
        self.load_button.pack(pady=10)

        self.detect_button = tk.Button(root, text="Detect chromosome", command=self.detect_objects)
